      ...
      if ((rowData >> (2 - col)) & 1) {
        ...
        for (int sy = 0; sy < 4 * scale; sy++) {
          for (int sx = 0; sx < 4 * scale; sx++) {
            vga_draw_pixel(buffer, x + col * 4 * scale + sx,
                           y + row * 4 * scale + sy, color);
          }
        }
      }
    }
      ...
      if ((rowData >> (2 - col)) & 1) {
        for (int sy = 0; sy < 2 * scale; sy++) {
          for (int sx = 0; sx < 2 * scale; sx++) {
            vga_draw_pixel(buffer, x + col * 2 * scale + sx,
                           y + row * 2 * scale + sy, color);
          }
        }
      }
    }
...
void vga_draw_string_simple(int x, int y, const char *str, uint32_t color,
                            int scale) {
  int cursor_x = x;
  for (int i = 0; str[i] != '\0'; i++) {
    if (str[i] == '\n') {
